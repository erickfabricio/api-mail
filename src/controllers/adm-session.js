...
const SESSION_TIME_30D = 2592000;
const SESSION_TIME_1Y = 31104000;
const SESSION_TIME_5Y = 155520000; //admin-finance

const SESSION_TIME = SESSION_TIME_24H;
...
module.exports = {
    ...
    login: async (req, res, next) => {
        ...
        if (user) { //Validating user existence
            if (user.state == 'Activo') { //Validating user status
                bcrypt.compare(req.body.password, user.hash, async function (err, result) {
                    if (result) {
                        ...
                        res.status(200).json({ ok: true, menssage: "Inicio de sesión correcto", token: token });
                    } else {
                        res.status(200).json({ ok: false, menssage: "Contraseña incorrecta" });
                    }
                })
            } else {
                res.status(200).json({ ok: false, menssage: "Usuario inactivo" });
            }
        } else {
            res.status(200).json({ ok: false, menssage: "Correo electrónico no registrado" });
        }
    },
